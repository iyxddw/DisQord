...
2. 按需添加“翻译”“审核”“固定文本”和“图片合成”模块。
3. 再选择目标会话并添加“发送目标”。
4. 从左到右连接模块。审核节点右侧上方是“过审”，下方是“未过”。
5. 填写蓝图名称并点击“保存并发布”。双向互通需要建立两条独立流水线。

推荐的常规路径：

```text
消息入口 → 翻译 → 审核 ──过审──→ 图片合成 → 发送目标
                       └─未过──→ 固定文本 → 图片合成 → 发送目标
```

审核模块的滑块表示允许的最高违规分数：模型分数小于或等于阈值时走“过审”，高于阈值时走
“未过”。提示词直接填写在翻译或审核模块下方，不再使用单独的全局提示词页面。固定文本模块会
替换当前处理文本，适合输出“内容未通过审核”等提示。

翻译、审核和固定文本只修改流水线中的文字。图片合成始终重新读取消息入口收到的头像、昵称、
附件、时间和回复信息，再将流水线当前文字渲染成 PNG。固定文本不会在半透明区域泄露原文字，
...
“运行日志”可以按 Debug、Info、Warn 和 Error 筛选，并记录入口消息、蓝图和节点、翻译原始
返回、审核原始评分、渲染结果、发送排队、平台发送成功与失败。日志不会记录 API Key。
