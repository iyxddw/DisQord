...
手机或窄屏打开蓝图时会自动切换成纵向卡片。卡片内可以编辑提示词、文本和记忆模式；使用上下按钮会
同时调整显示顺序和真实连接关系。新模块会自动追加到当前流程末尾。只有在桌面蓝图中确实没有连接的
孤立模块才会进入“未连接模块”，点击模块顶部的“入队”即可把它追加到当前路径底部。通过“连接到”
下拉框修改普通出口和审核分支；分支按钮只改变当前预览路径，不会改变已保存版本，修改完成后仍需点击
“保存并发布”。

模拟输入运行时，节点先显示浅绿色运行底层；更亮的绿色附层会快速推进到 50%，再先快后慢地接近完成。
只有当前节点完成时才显示 100%，并立即熄灭当前节点、点亮实际选中的下一节点。模拟输入点击“发送”后
直接显示 50%。“保存并运行”和“发布新版本”在请求期间会锁定并显示加载动画，避免重复提交。真实消息
不会因为模拟间隔设置而增加延迟。蓝图活动使用长轮询读取，页面保持打开时通常只会有一条等待中的活动
请求，而不会持续高频刷新接口。
