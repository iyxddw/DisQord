...
手机或窄屏打开蓝图时会自动切换成纵向卡片。卡片内可以编辑提示词、文本和记忆模式；长按拖拽手柄
即可“拿起”模块移动，拖到屏幕顶部或底部会自动滚动页面，也可以使用上下按钮调整位置。通过“连接到”
下拉框修改普通出口和审核分支；分支按钮只改变当前预览路径，不会改变已保存版本，修改完成后仍需点击
“保存并发布”。

模拟输入运行时，消息一到达节点，该节点就会变成浅绿色；更亮的绿色叠层会按模拟间隔从左向右推进，
节点完成后再显示处理结果。“保存并运行”和“发布新版本”在请求期间会锁定并显示加载动画，避免重复
提交。真实消息不会因为模拟间隔设置而增加延迟。蓝图活动使用长轮询读取，页面保持打开时通常只会有
一条等待中的活动请求，而不会持续高频刷新接口。
