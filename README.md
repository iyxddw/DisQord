...
脚本会检查与 `update.sh` 同级的 `central.env`、`qq.env` 和 `discord.env`，自动选择存在环境文件的部署角色，并在显示更新计划后倒数 5 秒开始。倒计时期间按任意键可进入原来的手动菜单，按 `q` 取消；未检测到环境文件时也会直接进入手动菜单。菜单使用方向键移动、空格选择、回车确认。构建成功后会自动执行所选角色对应的启动脚本，重启对应的 PM2 进程。
