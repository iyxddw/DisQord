...
QQ 客户端首次启动后会自动登记到中央面板“绑定会话”。先创建环境文件：
...
客户端在线后，在中央面板选择客户端上报的群名并发送验证码，到该 QQ 群读取验证码并回填。
同一客户端可以绑定多个群；更换群仍需重新验证。
...
客户端首次连接后自动出现在中央面板“绑定会话”。选择客户端上报的服务器和频道；中央端命令
该客户端向指定频道发送验证码，回填正确后状态变为“已验证”。同一客户端可以绑定多个频道；
更换服务器或频道必须重新验证。节点面板同样通过 SSH 隧道访问。
...
“聊天会话”页面保存已经配置过的群/频道，并支持备注和删除。新增会话统一在“绑定会话”完成：
从客户端自动上报的群/频道中选择，发送验证码并回填；同一客户端可绑定多个会话。只有验证成功的
会话可以加入蓝图。
